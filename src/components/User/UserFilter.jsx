...
const UserFilter = ({
  ...
  const onSubmit = (data) => {
    ...
    onApply({
      ...
      isActive: data.isActive === null ? "" : data.isActive.value,
    });

    onClose();
  };
  // ============================================================
  // LOAD PERSISTED FILTERS INTO DRAWER
  // ============================================================

  useEffect(() => {
    // Only sync values when drawer is opened
    if (!isOpen) return;

    // User option
    const selectedUser = initialFilters.userId
      ? {
          value: initialFilters.userId,
          label: initialFilters.userId,
        }
      : null;

    // Role option
    const selectedRole =
      roleOptions.find((option) => option.value === initialFilters.role) ||
      null;
    ...
    if (initialFilters.isActive !== "") {
      const activeValue =
        initialFilters.isActive === true || initialFilters.isActive === "true";

      selectedStatus =
        statusOptions.find((option) => option.value === activeValue) || null;
    }
