...
const BedShiftModal = ({ isOpen, onClose, client, getAvailableBeds }) => {
  const {
    control,
    handleSubmit,
    watch,
  ...
  // Clear previously selected bed because it may belong
  // to the previous property.
  // ============================================================
  const handlePropertyChange = (selectedOption) => {
    setValue("propertyId", selectedOption?.value || null, {
      shouldDirty: true,
      shouldValidate: true,
    });
  ...
  const businessMonthEnd = new Date(currentYear, currentMonth, 30);

  // End date maximum = 29
  const maxEndDate = new Date(currentYear, currentMonth, 30);
          ...
          <div>
            ...
            <p className="text-sm mt-1">
              ...
              <span className="font-bold">
                {client?.fullName} / {client?.propertyId?.propertyCode} /{" "}
                {client?.bedId?.bedNo}
              </span>
            </p>
          </div>
